# Jaccard:
# Hard negative mining: nega = 3 * positive default box
# Loss in regresstion task: MSE -> F.SmoothL1Loss
# Loss on classification (mylti class): F.CrossEntropy
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        #conf_data = (batch, dbox, classes)
        num_batch = loc_data.size(0)
        num_dbox = loc_data.size(1) #8732
        num_classes = conf_data.size(2)
        
        conf_t_label = torch.LongTensor(num_batch, num_dbox).to(device=self.device)
        loc_t = torch.Tensor(num_batch, num_dbox, 4).to(device=self.device)
        ...
        # SmoothL1Loss
        pos_mask = conf_t_label > 0  # lọai bỏ label là background 
        # loc_data( batch, 8732, 4)
        pos_idx = pos_mask.unsqueeze(pos_mask.dim()).expand_as(loc_data)
        
        # positive dbox, loc_data
        loc_p = loc_data[pos_idx].view(-1, 4) # du doan
        ...
        #loss_conf
        # crossEntropy
        batch_conf = conf_data.view(-1, num_classes) # (btach, box, 21) -> (batch * box, 21)
        loss_conf = F.cross_entropy(batch_conf, conf_t_label.view(-1), reduction='none')
        
        # hard negative mining
        num_pos = pos_mask.long().sum(1, keepdim=True)
        loss_conf = loss_conf.view(num_batch, -1) # torch.size([batch, 8732])
        
        _, loss_idx = loss_conf.sort(1, descending=True)
        _, idx_rank = loss_idx.sort(1)
        # idx_rank chinh la thong so để biết đc độ lơn loss nawmg ở vị trí bao nhiêu
        
        num_neg = torch.clamp(num_pos * self.neg_pos, max=num_dbox) 
        
        neg_mask = idx_rank < (num_neg).expand_as(idx_rank)
        
        # (numbatch, 8732) -> (batch, 8732, 21)
        pos_idx_mask = pos_mask.unsqueeze(2).expand_as(conf_data)
        neg_idx_mask = neg_mask.unsqueeze(2).expand_as(conf_data)
        
        conf_t_pre = conf_data[(pos_idx_mask + neg_idx_mask).gt(0)].view(-1, num_classes) # mạng dự đoán
        ...
        N = num_pos.sum() # số lượng positive
        loss_loc = loss_loc/N
